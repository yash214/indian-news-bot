...
Important:
- This provider is optional and disabled unless configured.
- It must never crash Macro Agent callers.
- FMP is mainly used for global macro / market context:
  USD/INR, gold, crude, US indices, economic calendar, and basic global indicators.
- India-specific macro data such as CPI/GDP/IIP should come from MOSPI.
- RBI/NSDL/CCIL remain the preferred India-specific providers.
"""
...
class FMPProvider:
    """Optional Financial Modeling Prep provider for macro context."""
    ...
    # These are mainly global/US macro indicators from FMP.
    # Do not treat them as India-specific macro data.
    ...
    def __init__(
        ...
        self.api_key = (
            api_key if api_key is not None else os.environ.get("FMP_API_KEY", "")
        ).strip() or None

        # Important: respect explicit enabled=True/False.
        # The old pattern `enabled or env` ignored explicit False.
        self.enabled = _env_bool("FMP_ENABLED", FMP_ENABLED) if enabled is None else bool(enabled)

        self.timeout_seconds = int(
            timeout_seconds
            if timeout_seconds is not None
            else os.environ.get("FMP_TIMEOUT_SECONDS") or FMP_TIMEOUT_SECONDS
        )

        self.cache_ttl_seconds = int(
            cache_ttl_seconds
            if cache_ttl_seconds is not None
            else os.environ.get("FMP_CACHE_TTL_SECONDS") or FMP_CACHE_TTL_SECONDS
        )
    ...
    def get_usd_inr(self) -> dict | None:
        if not self.is_configured():
            self._mark_not_configured_if_enabled()
            return None
    ...
    def get_crude(self) -> dict | None:
        ...
        # For India macro context, Brent is usually more relevant than WTI.
    ...
    def get_basic_macro_snapshot(self) -> dict:
        """Return basic global macro snapshot from FMP.

        Note:
        These FMP indicators are mostly global/US macro context.
        India CPI/GDP/IIP should be read from MOSPI, not this method.
        """
        if not self.is_configured():
            self._mark_not_configured_if_enabled()
            return {}
    ...
    def _request_json(self, path: str, params: dict | None = None) -> Any:
        if not self.is_configured():
            self._mark_not_configured_if_enabled()
            return None
        ...
        try:
            ...
            if response.status_code in {401, 403, 404, 429}:
                self.last_error = f"FMP HTTP {response.status_code} for {path}"
                return None
            ...
            try:
                ...
                self.last_error = f"FMP JSON parse error for {path}: {str(exc)[:240]}"
                return None
            ...
            self.last_error = f"FMP request error for {path}: {str(exc)[:240]}"
            return None
    ...
    def _normalize_quote_payload(
        ...
        *,
        ...
        change_pct = _safe_float(
            row.get(
                "changesPercentage",
                row.get("changePercent", row.get("percentageChange")),
            )
        )
    ...
    def _mark_not_configured_if_enabled(self) -> None:
        if self.enabled and not self.api_key:
            self.last_error = "FMP provider enabled but FMP_API_KEY is missing."
